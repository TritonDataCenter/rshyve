...
// used under MPL-2.0. Upstream: crates/bhyve-api/sys/src/vmm_data.rs
...
impl vdi_field_entry_v1 {
    /// Create a `vdi_field_entry_v1` from `ident` and `value`, with `_pad`
    /// set to zero.
...
// Time data for API versions before 11. VDC_VMM_TIME replaces it in
...
/// Guest TSC frequency measured by hrtime (not affected by wall clock changes)
