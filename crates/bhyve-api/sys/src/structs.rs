...
// used under MPL-2.0. Upstream: crates/bhyve-api/sys/src/structs.rs
...
impl vcpu_cpuid_entry {
    ...
    /// in matching.
    ///
    /// Sort a list of entries with this before loading them into the kernel
    /// VMM.
    ///
    /// ```
...
/// Copy a VM name into the array used by create and destroy requests. The
/// kernel does all other validation of the name.
...
// Flag values for `vm_create_req.flags`:
...
pub struct vmm_resv_target {
    ...
    /// The kernel changes the reservoir size in steps of at most this size.
    pub vrt_chunk_sz: size_t,

    /// Reservoir size after the operation. It equals the target size unless
...
// ── PCI passthrough (PPT) device structs ─────────────────────────

/// Bind/unbind a PPT device to/from a VM.
#[repr(C)]
#[derive(Copy, Clone, Default)]
...
/// Map/unmap PPT device MMIO into guest physical address space.
#[repr(C)]
#[derive(Copy, Clone, Default)]
...
/// Configure MSI interrupt delivery for a PPT device.
#[repr(C)]
#[derive(Copy, Clone, Default)]
...
/// Configure a single MSI-X vector for a PPT device.
#[repr(C)]
#[derive(Copy, Clone, Default)]
...
/// Query MSI/MSI-X interrupt limits for a PPT device.
#[repr(C)]
#[derive(Copy, Clone, Default)]
...
// ── PPT device ioctls (on /dev/pptN fd) ─────────────────────────

/// PPT device config space access, for PPT_CFG_READ and PPT_CFG_WRITE.
#[repr(C)]
#[derive(Copy, Clone, Default)]
pub struct ppt_cfg_io {
    pub pci_off: u64,
    pub pci_width: u32,
    pub pci_data: u32,
}

/// PPT device BAR query.
#[repr(C)]
#[derive(Copy, Clone, Default)]
pub struct ppt_bar_query {
    pub pbq_baridx: u32,
    pub pbq_type: u32,
    pub pbq_base: u64,
    pub pbq_size: u64,
...
// PPT BAR types, as reported in `ppt_bar_query.pbq_type`.
pub const PCI_ADDR_IO: u32 = 1;
pub const PCI_ADDR_MEM32: u32 = 2;
pub const PCI_ADDR_MEM64: u32 = 3;
...
mod tests {
    ...
    #[test]
    fn vm_munmap_layout() {
        assert_eq!(size_of::<vm_munmap>(), 16);
    }
}
